...
while(True):
    ...
    if len(faces)>0:
        ...
        UDAR = up_down_aspect_ratio(points)
        if c == 1:
            UD_AR_THRESH1 = UDAR - 7
            UD_AR_THRESH2 = UDAR + 5
            print(UDAR)  # For debug purpose
        ...
        #UDAR Checking
        if UDAR < UD_AR_THRESH1 or UDAR > UD_AR_THRESH2:
            UD_COUNTER += 1

            if UD_COUNTER >= UD_CONSEQ_FRAME1:
                if not ALARM1_ON:
                    ALARM1_ON = True
                    t = Thread(target=sound_alarm, args=(alarm_sound_path,))
                    t.start()
        ...
        cv2.putText(frame, "UDAR:{:.2f}".format(UDAR),(300,90),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,255,0),2)
